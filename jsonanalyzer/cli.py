# -*- coding: utf-8 -*-
"""
cli.py: Command-line interface for the JSON file analyzer.
"""
...
from .file_handler import analyze_json_file
from .report import generate_json_report
from .models import JsonAnalysisResult # For type hinting


def main() -> None:
    """Handles command-line arguments, runs analysis, and outputs JSON."""
    parser = argparse.ArgumentParser(
        description="Analyze the structure and statistics of a JSON file and output results as JSON.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Example Usage:
  python -m jsonanalyzer data.json
  python -m jsonanalyzer config.json -o report.json
  python -m jsonanalyzer input.json --copy --no-pretty

Outputs a JSON object detailing the file's structure, types, and basic stats."""
    )
    parser.add_argument("filepath", help="Path to the JSON file to analyze.")
    parser.add_argument(
        "-o", "--output",
        metavar="FILE",
    ...
    # --- Analysis ---
    # analyze_json_file handles reading, parsing, and analysis initiation
    analyzer_instance = analyze_json_file(filepath)

    # --- Report Generation ---
    # generate_json_report takes the analyzer instance and formats the final dict
    report_data: JsonAnalysisResult = generate_json_report(analyzer_instance)
    ...
    try:
        ...
        # Attempt to output minimal error JSON directly
        try:
            minimal_error = json.dumps({
                "filepath": analyzer_instance.results.get("filepath") or os.path.abspath(filepath),
                "analysis_error": analyzer_instance.results.get("analysis_error") or error_msg
                }, indent=2, ensure_ascii=False, sort_keys=True)
            print(minimal_error, file=sys.stderr)
        except Exception: # If even minimal fails, just print the original message
             pass # Original error already printed
    ...
    if copy_to_clipboard:
        if not HAS_PYPERCLIP:
            ...
            try:
                ...
                if not output_path and output_written: # Add newline if printing to stdout to separate confirmation
